...
Use this checklist before a local demo of the Balance Phase 1 MVP. It is intentionally focused on the current React/Vite local build for the Lessons 1 and 2 path, not the future hosted Firebase version.
...
- `[ ]` Refresh/resume after completion: refresh the browser and confirm the local account is still signed in, the course path shows Lesson 1 as completed/mastered, `One-Step Equations` is available, `Two-Step Equations` remains coming soon, and `View completion` returns to the Lesson 1 completion view.
...
## Path Unlock And Lesson 2 Manual Smoke Test

- `[ ]` Fresh path lock state: with a fresh local demo account before completing Lesson 1, confirm `Balancing Equations` is available, `One-Step Equations` is locked, and `Two-Step Equations` is labeled coming soon with no start/continue action.
...
- `[ ]` Coming soon gate: after Lesson 1 completion and again after Lesson 2 completion, confirm `Two-Step Equations` remains coming soon and cannot be started.
...
- `[ ]` Lesson 2 completion: advance through the summary screen and confirm the path shows `One-Step Equations` completed/mastered while `Two-Step Equations` still remains coming soon.
...
- `[ ]` Lessons 1 and 2 can be completed without Firebase or any hosted backend.
