...
- `[ ]` Confirm the copy states that local demo profiles and progress are stored in this browser, no passwords are collected, and Firebase Auth is required before production credentials.

## Lesson 1 Manual Smoke Test

- `[ ]` Local profile: choose `Start with a local demo account`, or create a local demo profile with an email/display name only, and confirm the app lands on the Algebra Foundations path.
...
- `[ ]` Local logout/resume: log out, resume with the same local demo email in the same browser profile, and confirm progress is still available without entering a password.
...
Deferred hosted checks, expected to fail or remain unchecked until Firebase/Supabase is wired into the runtime:
...
These hosted checks are deferred because the current Phase 1 backend is browser-local and the hosted adapter is not active for this MVP pass. They are not dropped from scope.
