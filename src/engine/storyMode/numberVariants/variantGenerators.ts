// Story Mode legacy number-variant generators.
//
// These produce randomized VARIANTS of bundled multi-step `input`/`sequence`/`operation-choice`
// steps via number-token substitution, each double-guarded with the REAL checkers so a wrong answer
// key can never ship (see the original module header for the full correctness argument). They are
// reachable only via the legacy branch of `src/story/rehydrateQuestion.ts`.
//
...
import { parseLinear, solveModel } from './linearParser'
import type { LinearModel, Slot } from './linearParser'
import { finalCoordinate, parseWalkMoves } from './textScanners'
