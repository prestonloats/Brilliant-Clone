          ...
          return (
            <button
              ...
              onClick={() => {
                ...
                // Correctness and the hint -> explanation -> reveal escalation live in the
                // engine (checkMcqStep), so MCQ stays uniform with the other assessed step
                // types instead of re-deriving the logic here.
                const result = checkMcqStep(step, option.id, nextAttempt)

                setSelectedId(option.id)
                setAttempts(nextAttempt)
                setSelectedFeedback(result.feedback)
                setReveal(result.correct ? '' : result.reveal ?? '')
                setRetryGuidance(result.retryGuidance ?? 'Choose another option to try again.')
